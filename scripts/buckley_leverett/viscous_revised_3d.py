...
results_dir = (
    pathlib.Path(__file__).parent.parent.parent / "results" / "viscous_revised_3d"
)
...
def compare_solvers(model_params: dict[str, Any], run_case: str) -> None:
    ...
    # Try solving the target problem with Newton solver first to check if it converges.
    _, converged = solve(
        BuckleyLeverettModel(model_params),
        final_time=FINAL_TIME,
        num_time_steps=1,
        appleyard_damping=True,
        physical_damping=False,
    )
    with (case_dir_final_time / "newton_convergence.txt").open("w") as f:
        f.write(f"Newton converged: {converged}")
    ...
    curvature_fig_reduced.savefig(  # type: ignore
        case_dir_final_time / "curvature_reduced.png",
        bbox_inches="tight",
    )
