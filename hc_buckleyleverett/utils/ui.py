"""Logging helpers and tqdm progress-bar wrappers."""
...
class DummyProgressBar:
    ...
    def __init__(self, *args, **kwargs):
        # Accept the same positional argument as tqdm.trange (an integer count).
        self._range = range(*args) if args else range(0)
        self._iter: Iterator[int] | None = None
...
try:
    ...
    from tqdm.contrib.logging import (  # type: ignore
        _get_first_found_console_logging_handler,  # type: ignore
        _TqdmLoggingHandler,  # type: ignore
        logging_redirect_tqdm,
    )

    progressbar_class: type[Any] = _tqdm  # type: ignore
except ImportError:
    progressbar_class: type[Any] = DummyProgressBar  # type: ignore
